...
The **Default Space is the one exception**, on both halves of that rule. It has zero memberships by
construction, so nobody can ever be a member of it; its standing comes instead from the instance
administrator (`instance_settings.owner_user_id`) or its own `spaces.owner_user_id`, who is answered
as though holding its owner role, with `currentMembership.id` **null** because no membership row
exists. Any other signed-in caller receives `403`, not `404`: the Default Space has a fixed id and
exists on every instance, so concealing it would conceal nothing while misreporting a settled
authorization answer as a lookup miss. Bearer credentials are unaffected — they still receive `404`
for every Space including this one.
...
| -------- | ------------------------------------------------------------------------------- | ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/spaces`                                                                | —                         | Spaces the caller is a member of, plus Spaces holding a resource the caller can read. Each entry carries `currentMembership` (null when the caller only reaches it through a resource; a synthetic entry with a null `id` for the Default Space's administrator) and `resourceCounts`. The Default Space is listed for its administrator even when the instance holds no resources yet. A `currentMembership: null` entry is a container label only — `id`, `name`, `isDefault` — and withholds `description`, `ownerUserId`, and audit fields. |
