...
/**
 * Typed field-value mechanics shared by the breakdown and tracker record grids. Both surfaces
 * store one value row per (record, field) pair over the same column vocabulary — scalar columns
 * keyed by field type, a single-select `optionId`, and multi-select join rows — so the
 * contract-to-column mapping lives here exactly once.
 */
...
export function valueData(value: FieldValueInput, validOptionIds: string[]): FieldValueColumns {
  switch (value.type) {
    case 'text':
    case 'long_text':
