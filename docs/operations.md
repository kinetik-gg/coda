...
```sh
export CODA_IMAGE='ghcr.io/kinetik-gg/coda@sha256:61af3915e7933f87f091a10b6f60438f7db9e79084175e304d49255542129182'
...
| `THROTTLE_DEFAULT_LIMIT`        | `120`                 | App-wide per-IP request budget over a fixed 60-second window, `1`–`100000`, for every route not already narrowed by its own stricter limit (for example `SCREENPLAY_PREAUTH_MAX_PER_CLIENT`). Raise only for a disposable, non-internet-facing environment that legitimately drives more traffic through one shared address; never disable the guard. |
...
| `PRE_UPGRADE_BACKUP`               | `on`      | `on` or `off`. When `on`, an initialized instance writes an automatic signed archive under `backups/pre-upgrade/` before applying pending migrations. Needs `CONFIG_ENCRYPTION_KEY`: without it an initialized instance with pending migrations **refuses to migrate** and stops at the boot diagnostic page. Set `PRE_UPGRADE_BACKUP=off` to migrate deliberately without a safety archive. See [Pre-upgrade auto-backup](#pre-upgrade-auto-backup). |
