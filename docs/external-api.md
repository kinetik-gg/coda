...
| -------- | ------------------------------------------------------------------------------- | ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/spaces`                                                                | —                         | Spaces the caller is a member of, plus Spaces holding a resource the caller can read. Each entry carries `currentMembership` (null when the caller only reaches it through a resource) and `resourceCounts`. A `currentMembership: null` entry is a container label only — `id`, `name`, `isDefault` — and withholds `description`, `ownerUserId`, and audit fields. |
