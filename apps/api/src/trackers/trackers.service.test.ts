...
describe('TrackersService', () => {
  ...
  it('soft-deletes with the deletion triple and requires manage_tracker_settings', async () => {
    const tx = {
      ...
      activityEvent: { create: vi.fn().mockResolvedValue({}) },
      trackerInvitation: { updateMany: vi.fn().mockResolvedValue({ count: 0 }) },
      instanceInvitation: { updateMany: vi.fn().mockResolvedValue({ count: 0 }) },
    };
    const permissions = {
  ...
  it('revokes pending tracker invitations inside the trash transaction', async () => {
    const trackerInvitation = { updateMany: vi.fn().mockResolvedValue({ count: 1 }) };
    const tx = {
      tracker: { updateMany: vi.fn().mockResolvedValue({ count: 1 }) },
      activityEvent: { create: vi.fn().mockResolvedValue({}) },
      trackerInvitation,
    ...
    // Same revocation shape as project trash: only PENDING rows without an existing revocation
    // stamp flip to REVOKED at the deletion timestamp.
    expect(trackerInvitation.updateMany).toHaveBeenCalledWith({
      where: { trackerId: 'tracker-id', status: 'PENDING', revokedAt: null },
      data: { status: 'REVOKED', revokedAt: result.deletedAt },
    });
  });

  it('revokes pending instance invitations embedding this tracker', async () => {
    const instanceInvitation = { updateMany: vi.fn().mockResolvedValue({ count: 2 }) };
    const tx = {
      tracker: { updateMany: vi.fn().mockResolvedValue({ count: 1 }) },
      activityEvent: { create: vi.fn().mockResolvedValue({}) },
      trackerInvitation: { updateMany: vi.fn().mockResolvedValue({ count: 0 }) },
    ...
    // Instance invitations always carry trackerId alongside trackerRoleId when they embed a
    // tracker grant (instance-management validates the pairing), so keying on trackerId covers
    // every embedded variant.
    expect(instanceInvitation.updateMany).toHaveBeenCalledWith({
      where: { trackerId: 'tracker-id', status: 'PENDING', revokedAt: null },
      data: { status: 'REVOKED', revokedAt: result.deletedAt },
    });
  });
