...
export const externalOpenApiSchemas: JsonObject = {
  ...
  SourceReference: {
    ...
    properties: {
      ...
      position: rank,
      resolution: { type: 'string', enum: ['pinned', 'unavailable', 'unpinned'] },
      pin: {
        oneOf: [{ type: 'null' }, { $ref: '#/components/schemas/SourceReferenceRevisionPin' }],
      },
      staleness: { oneOf: [{ type: 'null' }, { type: 'string', enum: ['current', 'stale'] }] },
    },
  },
  SourceReferenceRevisionPin: {
    type: 'object',
    required: [
    ...
    properties: {
      itemSourceReferenceId: uuid,
      screenplayId: uuid,
      screenplayRevisionId: uuid,
      screenplayVersion: { type: 'integer', minimum: 1 },
      source: {
        type: 'object',
        required: ['start', 'end'],
        properties: {
      ...
      sourceTextHash: { type: 'string', pattern: '^[0-9a-f]{64}$' },
      sourceOffsetUnit: { type: 'string', enum: ['utf16-code-unit'] },
      createdById: uuid,
      updatedById: uuid,
      createdAt: timestamp,
      updatedAt: timestamp,
