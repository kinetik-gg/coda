...
describe('credential-security', () => {
  ...
  it('derives bearerAuth for the tracker families the guard allowlist covers', () => {
    ...
    expect(isCredentialAllowedForTemplatedPath('GET', '/api/v1/trackers/{trackerId}/records')).toBe(
      true,
    );
    expect(
      isCredentialAllowedForTemplatedPath(
    ...
    expect(
      isCredentialAllowedForTemplatedPath('GET', '/api/v1/trackers/{trackerId}/activity'),
    ).toBe(true);
  ...
  it('keeps the session-only tracker families out of the bearer derivation', () => {
    expect(isCredentialAllowedForTemplatedPath('GET', '/api/v1/trackers')).toBe(false);
    expect(
    ...
    expect(isCredentialAllowedForTemplatedPath('DELETE', '/api/v1/trackers/{trackerId}')).toBe(
      false,
    );
  });
