...
# Every stack in this lane shares one key, generated per run exactly as the documentation tells an
# operator to generate it, and never committed. Carrying the same key is what an operator must do to
# move data between instances anyway. Issue #268: an initialized instance with pending migrations
# refuses to migrate without that key, so the old-image-to-candidate upgrade below now exercises the
# real pre-upgrade safety-backup path rather than the key-less exemption it used to inherit.
