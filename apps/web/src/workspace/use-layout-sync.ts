...
/**
 * The wire shape of a saved workspace layout tier and the combined personal/default read.
 * Shared verbatim by the breakdown and tracker workspaces, whose layout endpoints differ only
 * in URL prefix.
 */
...
/**
 * Owns the self-healing sync between a member's personal workspace layout and the published
 ...
 *
 * One engine serves every workspace surface: callers pass only the resource base URL
 * (`/api/v1/projects/:id/workspace-layout`, `/api/v1/trackers/:id/workspace-layout`), because
 * the breakdown and tracker layout contracts are byte-for-byte the same wire vocabulary.
 */
export function useWorkspaceLayoutSync(baseUrl: string, stored: LayoutResponse | undefined) {
  ...
  const refetchLayouts = useCallback(async (): Promise<LayoutResponse> => {
    const latest = await api<LayoutResponse>(baseUrl);
  ...
  const reset = useCallback(
    () =>
      enqueue(() =>
        syncWithRetry(
          () =>
            api<StoredLayout>(`${baseUrl}/reset`, {
              method: 'POST',
    ...
    [enqueue, baseUrl, syncWithRetry, applyStoredPersonal],
  );
  ...
  }, [baseUrl, refetchLayouts, pushToast]);
